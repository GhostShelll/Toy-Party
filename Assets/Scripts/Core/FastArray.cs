...
namespace com.jbg.core
{
    ...
    public class FastArray<T> : ICloneable, IList<T> where T : class
    {
        ...
        {
            if (startIdx < 0 || this.count < startIdx)
            ...
            if (count == 0 || this.count == startIdx)
                return;

            if (count < 0 || this.count < startIdx + count)
        ...
        {
            FastArray<T> newArr = new(src.capacity);
        ...
        protected PoolComparer comparer = new();
